...
const NPSHistogram: React.FC<NPSHistogramProps> = ({
	...
	const { maxDensity, bars } = useMemo(() => {
		if (!data || data.length === 0) {
			return { maxDensity: 1, bars: [] };
		}
		...
		return { maxDensity, bars };
	}, [data]);

	useEffect(() => {
		const canvas = canvasRef.current;
		if (!canvas || !bars.length) {
			console.log("NPSHistogram: No canvas or no bars", {
				canvas: !!canvas,
		...
		if (Math.floor(currentTime * 2) % 1 === 0) {
			console.log("NPSHistogram: Drawing", {
				bars: bars.length,
				maxDensity,
				currentTime,
				totalTime,
		...
		bars.forEach((item, index) => {
			...
			const height = Math.max(
				2,
				(item.density / maxDensity) * (canvasHeight - 8),
			); // Minimum 2px height
			...
			if (item.density >= 35)
				color = "#000000"; // black
			else if (item.density >= 30)
				color = "#3b82f6"; // blue
			else if (item.density >= 20)
				color = "#ef4444"; // red
	...
	return (
		<div
			className="w-full h-20 bg-base-200/20 rounded-lg  my-2 relative"
			style={{ zIndex: 100 }}
		>
			<canvas
				ref={canvasRef}
