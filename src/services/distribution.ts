...
export function calcNPS(map: Beatmap): number | null {
  ...
  const drainTimeMs = map.hitObjects[map.hitObjects.length - 1].startTime - map.hitObjects[0].startTime;
  if (drainTimeMs <= 0) {
...
export function calcNPSRangeByTime(map: Beatmap, startTime: number, endTime: number): number | null {
  ...
  const count = endIdx === -1 ? map.hitObjects.length - startIdx : endIdx - startIdx;
...
export function calcNPSRangeByHitObjects(
  ...
  const [minIdx, maxIdx] = startIdx <= endIdx ? [startIdx, endIdx + 1] : [endIdx, startIdx + 1];
  const count = maxIdx - minIdx;
  
  return count / (drainTimeMs / MS_TO_SEC);
}

export function calcDistribution(map: Beatmap, tParts: number): number[] | null {
  ...
  for (let part = 0; part < tParts; part++) {
    ...
    const startIdx = map.hitObjects.findIndex(h => h.startTime >= partStartTime);
    
    // Pour la dernière partie, inclure la dernière note avec <=
    const endIdx = part === tParts - 1 
...
export function calcDistribution2(map: Beatmap, tParts: number): number[] | null {
  if (tParts <= 0 || map.hitObjects.length === 0) {
    return null;
  }
  ...
  for (const hitObject of map.hitObjects) {
    ...
    const index = Math.min(Math.floor(relativeTime * invPartDuration), partSize - 1);
    counts[index]++;
  }
...
export function calcDistributionSmart(map: Beatmap, tParts: number): number[] | null {
  if (tParts <= 0 || map.hitObjects.length === 0) {
    return null;
  }
