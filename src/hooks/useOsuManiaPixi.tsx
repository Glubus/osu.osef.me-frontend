...
class Note {
  ...
  update(deltaTime: number, scrollSpeed: number, scrollDirection: string, canvasHeight: number) {
...
export const useOsuManiaPixi = ({
  ...
  useEffect(() => {
    ...
    const receptorsX = Array.from({ length: lanes }, (_, i) => i * laneWidth + laneWidth / 2);
    ...
    const updateNotesProgressively = () => {
      ...
      for (const hitObject of hitObjects) {
        ...
        if (timestamp >= currentTime * 1000 - 2000 && timestamp <= currentTime * 1000 + 6000) {
          const startY = scrollDirectionRef.current === "down"
            ? canvasHeight + 125 - (timestamp - currentTime * 1000) * (scrollSpeed / 16)
      ...
      if (drawRef.current.length > 0 && Math.floor(currentTime * 2) % 1 === 0) {
        console.log(`Notes visibles: ${drawRef.current.length}, Direction: ${scrollDirectionRef.current}, Speed: ${scrollSpeed}`);
      }
    };
    ...
    const animate = (currentTimestamp: number) => {
      ...
      drawRef.current.forEach(note => {
        note.update(deltaTime, scrollSpeed, scrollDirectionRef.current, canvasHeight);
